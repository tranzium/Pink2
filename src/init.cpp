...
bool AppInit(int argc, char* argv[])
{
    ...
    try
    {
        ...
        if (mapArgs.count("-?") || mapArgs.count("--help"))
        {
            ...
            std::string strUsage = _("Pinkcoin version") + " " + FormatFullVersion() + "\n\n" +
                _("Usage:") + "\n" +
                  "  pinkcoind [options]                     " + "\n" +
                  "  pinkcoind [options] <command> [params]  " + _("Send command to -server or pinkcoind") + "\n" +
                  "  pinkcoind [options] help                " + _("List commands") + "\n" +
                  "  pinkcoind [options] help <command>      " + _("Get help for a command") + "\n";
...
{
    uiInterface.ThreadSafeMessageBox(str, _("Pinkcoin"), CClientUIInterface::OK | CClientUIInterface::MODAL);
    return false;
}

bool static InitWarning(const std::string &str)
{
    uiInterface.ThreadSafeMessageBox(str, _("Pinkcoin"), CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
    return true;
}
...
std::string HelpMessage()
{
    string strUsage = _("Options:") + "\n" +
        ...
        "  -rpcport=<port>        " + _("Listen for JSON-RPC connections on <port> (default: 9135 or testnet: 19135)") + "\n" +
        ...
        "  -startfrom=<n>         " + _("Resync blockchain from a specific block number") +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. Pinkcoin is shutting down."));
    ...
    if (strWalletFileName != std::filesystem::path(strWalletFileName).filename().string())
        return InitError(strprintf(_("Wallet %s resides outside data directory %s."), strWalletFileName.c_str(), strDataDir.c_str()));

    // strStakeDBFileName must be a plain filename without a directory
    if (strStakeDBFileName != std::filesystem::path(strStakeDBFileName).filename().string())
    ...
    if (fDaemon)
        fprintf(stdout, "Pinkcoin server starting\n");
    ...
    if (std::filesystem::exists(GetDataDir() / strStakeDBFileName))
    {
    ...
    if (!targetFPOS)
    {
        if (valSplitThreshold >= valCombineThreshold)
        {
    ...
    if (nLoadWalletRet != DB_LOAD_OK)
    {
        ...
        else if (nLoadWalletRet == DB_NONCRITICAL_ERROR)
        {
            ...
            strErrors << _("Error loading wallet.dat: Wallet requires newer version of Pinkcoin") << "\n";
        else if (nLoadWalletRet == DB_NEED_REWRITE)
        {
            strErrors << _("Wallet needed to be rewritten: restart Pinkcoin to complete") << "\n";
