...
std::array<std::atomic<int>, THREAD_MAX> vnThreadsRunning;
    ...
    while (true)
    {
        ...
        else if (nBytes <= 0)
        {
            ...
            else
            {
                // socket error
                int nErr = WSAGetLastError();
                LogPrint(BCLog::NET, "recv failed: %d\n", nErr);
                return false;
            }
        }
    }
...
bool GetMyExternalIP2(const CService& addrConnect, const char* pszGet, const char* pszKeyword, CNetAddr& ipRet)
{
    ...
    while (RecvLine(hSocket, strLine))
    {
        if (strLine.empty()) // HTTP response is separated from headers by blank line
        {
            ...
            while (!strLine.empty() && isspace(strLine[strLine.size()-1]))
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest)
{
    ...
    LogPrint(BCLog::NET, "trying connection %s lastseen=%.1fhrs\n",
        ...
        pszDest ? 0 : static_cast<double>(GetAdjustedTime() - addrConnect.nTime)/3600.0);
...
void CNode::PushVersion()
{
    ...
    RAND_bytes(reinterpret_cast<unsigned char*>(&nLocalHostNonce), sizeof(nLocalHostNonce));
...
bool CNode::Misbehaving(int howmuch)
{
    ...
    if (nMisbehavior >= GetArg("-banscore", 100))
    {
        ...
        LogPrint(BCLog::NET, "Misbehaving: %s (%d -> %d)\n", addr.ToString().c_str(), nMisbehavior-howmuch, nMisbehavior);
    return false;
}
    ...
    while (it != pnode->vSendMsg.end()) {
        ...
        if (nBytes > 0) {
            ...
            if (nBytes < 0) {
                ...
                {
                    LogPrint(BCLog::NET, "socket send error %d\n", nErr);
...
void ThreadSocketHandler()
{
    ...
    while (true)
    {
        ...
        if (nSelect == SOCKET_ERROR)
        {
            ...
            {
                int nErr = WSAGetLastError();
                LogPrint(BCLog::NET, "socket select error %d\n", nErr);
        ...
        if (hListenSocket != INVALID_SOCKET && FD_ISSET(hListenSocket, &fdsetRecv))
        {
            ...
            SOCKET hSocket = accept(hListenSocket, reinterpret_cast<struct sockaddr*>(&sockaddr), &len);
            CAddress addr;
            int nInbound = 0;
            ...
            if (hSocket == INVALID_SOCKET)
            {
                ...
                if (nErr != WSAEWOULDBLOCK)
                    LogPrint(BCLog::NET, "socket error accept failed: %d\n", nErr);
            }
        ...
        for (CNode* pnode : vNodesCopy)
        {
            ...
            if (FD_ISSET(pnode->hSocket, &fdsetRecv) || FD_ISSET(pnode->hSocket, &fdsetError))
            {
                ...
                if (lockRecv)
                {
                    if (pnode->GetTotalRecvSize() > ReceiveFloodSize()) {
                        if (!pnode->fDisconnect)
                            LogPrint(BCLog::NET, "socket recv flood control disconnect (%u bytes)\n", pnode->GetTotalRecvSize());
                        pnode->CloseSocketDisconnect();
                    }
                    else {
                        ...
                        else if (nBytes < 0)
                        {
                            ...
                            {
                                if (!pnode->fDisconnect)
                                    LogPrint(BCLog::NET, "socket recv error %d\n", nErr);
                                pnode->CloseSocketDisconnect();
                            }
                        }
                    }
            ...
            if (nTime - pnode->nTimeConnected > 60)
            {
                ...
                {
                    LogPrint(BCLog::NET, "socket sending timeout: %" PRId64 "s\n", nTime - pnode->nLastSend);
                    pnode->fDisconnect = true;
                }
                else if (nTime - pnode->nLastRecv > (pnode->nVersion > BIP0031_VERSION ? TIMEOUT_INTERVAL : 90*60))
                {
                    LogPrint(BCLog::NET, "socket receive timeout: %" PRId64 "s\n", nTime - pnode->nLastRecv);
                    pnode->fDisconnect = true;
                }
...
void ThreadMapPort()
{
    ...
    RenameThread("pinkcoin-UPnP");
    ThreadCountGuard guard(THREAD_UPNP);
...
void ThreadDNSAddressSeed()
{
    ...
    RenameThread("pinkcoin-dnsseed");
    ThreadCountGuard guard(THREAD_DNSSEED);
...
void ThreadDumpAddress()
{
    ...
    RenameThread("pinkcoin-adrdump");
    ThreadCountGuard guard(THREAD_DUMPADDRESS);
...
static void ThreadStakeMiner(CWallet* pwallet)
{
    ...
    ThreadCountGuard guard(THREAD_STAKE_MINER);
    StakeMiner(pwallet);
    LogPrintf("ThreadStakeMiner exiting, %d threads remaining\n", vnThreadsRunning[THREAD_STAKE_MINER].load());
}

void ThreadOpenConnections()
{
    // Make this thread recognisable as the connection opening thread
    RenameThread("pinkcoin-opencon");
    ThreadCountGuard guard(THREAD_OPENCONNECTIONS);
    LogPrintf("ThreadOpenConnections started\n");

    // Connect to specific addresses
    if (mapArgs.count("-connect") && !mapMultiArgs["-connect"].empty())
    {
...
void ThreadOpenAddedConnections()
{
    // Make this thread recognisable as the connection opening thread
    RenameThread("pinkcoin-opencon");
    ThreadCountGuard guard(THREAD_ADDEDCONNECTIONS);
...
void ThreadMessageHandler()
{
    ...
    RenameThread("pinkcoin-msghand");
    ThreadCountGuard guard(THREAD_MESSAGEHANDLER);
...
bool BindListenPort(const CService &addrBind, std::string& strError)
{
    ...
    SOCKET hListenSocket = socket((reinterpret_cast<struct sockaddr*>(&sockaddr))->sa_family, SOCK_STREAM, IPPROTO_TCP);
    ...
    setsockopt(hListenSocket, SOL_SOCKET, SO_NOSIGPIPE, reinterpret_cast<void*>(&nOne), sizeof(int));
#endif

#ifndef WIN32
    // Allow binding if the port is still in TIME_WAIT state after
    // the program was closed and restarted.  Not an issue on windows.
    setsockopt(hListenSocket, SOL_SOCKET, SO_REUSEADDR, reinterpret_cast<void*>(&nOne), sizeof(int));
#endif
    ...
    if (addrBind.IsIPv6()) {
#ifdef IPV6_V6ONLY
#ifdef WIN32
        setsockopt(hListenSocket, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<const char*>(&nOne), sizeof(int));
#else
        setsockopt(hListenSocket, IPPROTO_IPV6, IPV6_V6ONLY, reinterpret_cast<void*>(&nOne), sizeof(int));
#endif
#endif
#ifdef WIN32
        ...
        setsockopt(hListenSocket, IPPROTO_IPV6, nParameterId, reinterpret_cast<const char*>(&nProtLevel), sizeof(int));
#endif
    }

    if (::bind(hListenSocket, reinterpret_cast<struct sockaddr*>(&sockaddr), len) == SOCKET_ERROR)
    {
        int nErr = WSAGetLastError();
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Pinkcoin is probably already running."), addrBind.ToString().c_str());
        else
...
void StartNode()
{
    ...
    if (semOutbound == nullptr) {
        ...
        int nMaxOutbound = std::min(MAX_OUTBOUND_CONNECTIONS, static_cast<int>(GetArg("-maxconnections", 125)));
    ...
    if (!GetBoolArg("-dnsseed", true))
        ...
        if (!NewThread(ThreadDNSAddressSeed))
