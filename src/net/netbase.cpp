...
{
    LogPrint(BCLog::NET, "SOCKS4 connecting %s\n", addrDest.ToString().c_str());
    if (!addrDest.IsIPv4())
    {
        closesocket(hSocket);
    ...
    LogPrint(BCLog::NET, "SOCKS4 connected %s\n", addrDest.ToString().c_str());
    return true;
}
...
bool static ConnectSocketDirectly(const CService &addrConnect, SOCKET& hSocketRet, int nTimeout)
{
    ...
    if (connect(hSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (WSAGetLastError() == WSAEINPROGRESS || WSAGetLastError() == WSAEWOULDBLOCK || WSAGetLastError() == WSAEINVAL)
        {
            ...
            if (nRet == 0)
            {
                LogPrint(BCLog::NET, "connection timeout\n");
                closesocket(hSocket);
                return false;
            }
            if (nRet == SOCKET_ERROR)
            {
                LogPrint(BCLog::NET, "select() for connection failed: %i\n",WSAGetLastError());
                closesocket(hSocket);
                return false;
            }
...
#endif
            {
                LogPrint(BCLog::NET, "getsockopt() for connection failed: %i\n",WSAGetLastError());
                closesocket(hSocket);
                return false;
            }
            if (nRet != 0)
            {
                LogPrint(BCLog::NET, "connect() failed after select(): %s\n",strerror(nRet));
                closesocket(hSocket);
                return false;
            }
        }
...
#endif
        {
            LogPrint(BCLog::NET, "connect() failed: %i\n",WSAGetLastError());
            closesocket(hSocket);
            return false;
        }
    }
