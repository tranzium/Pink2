...
public:
    ...
    void print() const
    {
        LogPrint(BCLog::STAKE, "COrphan(hash=%s, dPriority=%.1f, dFeePerKb=%.1f)\n",
               ptx->GetHash().ToString().substr(0,10).c_str(), dPriority, dFeePerKb);
        for (uint256 hash : setDependsOn)
            LogPrint(BCLog::STAKE, "   setDependsOn %s\n", hash.ToString().substr(0,10).c_str());
    }
};
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake, int64_t* pFees)
{
    ...
    pblock->nBits = GetNextTargetRequired(pindexPrev, fProofOfStake, rawtime);
    ...
    int64_t nFees = 0;
    {
        ...
        while (!vecPriority.empty())
        {
            ...
            {
                LogPrint(BCLog::STAKE, "priority %.1f feeperkb %.1f txid %s\n",
        ...
        LogPrint(BCLog::STAKE, "CreateNewBlock(): total size %" PRIu64"\n", nBlockSize);
...
void IncrementExtraNonce(CBlock* pblock, CBlockIndex* pindexPrev, unsigned int& nExtraNonce)
{
    ...
    pblock->vtx[0].vin[0].scriptSig = (CScript() << nHeight << CScriptNum(static_cast<int64_t>(nExtraNonce)).getvch()) + COINBASE_FLAGS;
...
//            fTryToSync = false;
            ...
            /*
             * If we have at least 3 peers and the same number or more blocks than our peers
            ...
            if ((vNodes.size() < static_cast<int>(MIN_PEERS) && !fTestNet) || nBestHeight < GetNumBlocksOfPeers())
            {
                ...
                fTryToSync = false;     // we only want to mine once we know for sure we're synced to the network.
            }
        }
