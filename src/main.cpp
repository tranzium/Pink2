...
// SetBestChain(CBlockLocator) and UpdatedTransaction moved to blockprocessing.cpp
...
bool AddOrphanTx(const CTransaction& tx)
{
    ...
    LogPrint(BCLog::CONSENSUS, "stored orphan tx %s (mapsz %" PRIszu ")\n", hash.ToString().substr(0,10).c_str(),
...
bool CTransaction::AreInputsStandard(const MapPrevTx& mapInputs) const
{
    ...
    for (unsigned int i = 0; i < vin.size(); i++)
    {
        ...
        if (stack.size() != static_cast<unsigned int>(nArgsExpected))
            return false;
    }
...
int CMerkleTx::SetMerkleBranch(const CBlock* pblock)
{
    ...
    for (nIndex = 0; nIndex < static_cast<int>(pblock->vtx.size()); nIndex++)
        if (pblock->vtx[nIndex] == *(CTransaction*)this)
            break;
    if (nIndex == static_cast<int>(pblock->vtx.size()))
    {
        vMerkleBranch.clear();
        nIndex = -1;
...
int64_t CTransaction::GetMinFee(unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes) const
{
    ...
    int64_t nMinFee = (1 + static_cast<int64_t>(nBytes) / 1000) * nBaseFee;
                        ...
                        bool* pfMissingInputs)
{
    ...
    LogPrint(BCLog::MEMPOOL, "CTxMemPool::accept() : accepted %s (poolsz %" PRIszu ")\n",
           hash.ToString().substr(0,10).c_str(),
           mapTx.size());
    return true;
}
...
// CTxMemPool CRUD methods moved to mempool.cpp
...
int CMerkleTx::GetBlocksToMaturity() const
{
    if (!(IsCoinBase() || IsCoinStake()))
        return 0;
    return std::max(0, (nCoinbaseMaturity + 10) - GetDepthInMainChain());
}
...
int CTxIndex::GetDepthInMainChain() const
{
    ...
    auto mi = mapBlockIndex.find(block.GetHash(true));
    if (mi == mapBlockIndex.end())
        return 0;
    CBlockIndex* pindex = mi->second;
    if (!pindex || !pindex->IsInMainChain())
...
CBlockIndex* pblockindexFBBHLast = nullptr;
...
// Reward, difficulty, and flash-stake functions moved to rewards.cpp



// CheckProofOfWork moved to validation.cpp
...
// InvalidChainFound, Reorganize, SetBestChainInner, SetBestChain,
// AddToBlockIndex, AcceptBlock, GetModTrust, GetBlockTrust
// moved to blockprocessing.cpp
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    if (pcheckpoint && pblock->hashPrevBlock != hashBestChain && !Checkpoints::WantedByPendingSyncCheckpoint(hash))
    {
        ...
        {
            if (IsFlashStake(pblock->nTime))
                bnRequired.SetCompact(ComputeMinStake(GetLastBlockIndex2(pcheckpoint, true)->nBits, deltaTime, pblock->nTime));
            else
                bnRequired.SetCompact(ComputeMinStake(GetLastBlockIndex2(pcheckpoint, false)->nBits, deltaTime, pblock->nTime));
        }
        else
    ...
    if(pindexBest->nTime < nTimeV221 && heightDiff > -1 && !Checkpoints::WantedByPendingSyncCheckpoint(hash)) {  //  Tuesday, September 19, 2018 7:00:00 AM UTC, can get rid of the time after the fork as it only applies to live blockchain updates.
        LogPrint(BCLog::CONSENSUS, "We're checking this one out. \n");
    ...
    LogPrint(BCLog::CONSENSUS, "ProcessBlock: ACCEPTED\n");
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    if (nSearchTime > nLastCoinStakeSearchTime)
    {
        ...
        {
            if (txCoinStake.nTime >= std::max(pindexBest->GetPastTimeLimit()+1, PastDrift(pindexBest->GetBlockTime())))
            {
                // make sure coinstake would meet timestamp protocol
                //    as it would be the same as the block timestamp
                vtx[0].nTime = nTime = txCoinStake.nTime;
                nTime = std::max(pindexBest->GetPastTimeLimit()+1, GetMaxTransactionTime());
                nTime = std::max(GetBlockTime(), PastDrift(pindexBest->GetBlockTime()));
...
// CBlock::CheckBlockSignature moved to validation.cpp
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        LogPrintf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "Pinkcoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 0 << CScriptNum(42).getvch() << std::vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
...
bool LoadExternalBlockFile(FILE* fileIn)
{
    ...
    LogPrintf("Loaded %d blocks from external file in %" PRId64 "ms\n", nLoaded, GetTimeMillis() - nStart);
    return nLoaded > 0;
}
...
bool static ProcessMessage(CNode* pfrom, std::string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version")
    {
        ...
        if (pfrom->nVersion < MIN_PEER_PROTO_VERSION && pindexBest->nHeight > 817990)
        {
            // disconnect from peers older than this proto version
            LogPrintf("partner %s using obsolete version %i; disconnecting\n", pfrom->addr.ToString().c_str(), pfrom->nVersion);
            pfrom->fDisconnect = true;
        ...
        {
            LogPrintf("partner %s does not have current time provided by NTP. "
        ...
        for (CAddress& addr : vAddr)
        {
            ...
            if (addr.nTime > nSince && !pfrom->fGetAddr && vAddr.size() <= 10 && addr.IsRoutable())
            {
                // Relay to a limited number of other nodes
                {
                    ...
                    hashRand = Hash(CharCast(hashRand), CharEnd(hashRand));
                    ...
                    for (CNode* pnode : vNodes)
                    {
                        ...
                        hashKey = Hash(CharCast(hashKey), CharEnd(hashKey));
    ...
    else if (strCommand == "inv")
    {
        ...
        for (unsigned int nInv = 0; nInv < vInv.size(); nInv++)
        {
            ...
            LogPrint(BCLog::NET, "  got inventory: %s  %s\n", inv.ToString().c_str(), fAlreadyHave ? "have" : "new");
            ...
            if (!fAlreadyHave)
                ...
                LogPrint(BCLog::NET, "force request: %s\n", inv.ToString().c_str());
            }
    ...
    else if (strCommand == "getdata")
    {
        ...
        LogPrint(BCLog::NET, "received getdata (%" PRIszu " invsz)\n", vInv.size());

        for (const CInv& inv : vInv)
        {
            if (fShutdown)
                return true;
            LogPrint(BCLog::NET, "received getdata for: %s\n", inv.ToString().c_str());
    ...
    else if (strCommand == "getblocks")
    {
        ...
        if (pindex)
            pindex = pindex->pnext;
        int nLimit = 500;
        LogPrint(BCLog::NET, "getblocks %d to %s limit %d\n", (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str(), nLimit);
    ...
    else if (strCommand == "getheaders")
    {
        ...
        int nLimit = 2000;
        LogPrint(BCLog::NET, "getheaders %d to %s\n", (pindex ? pindex->nHeight : -1), hashStop.ToString().substr(0,20).c_str());
        for (; pindex; pindex = pindex->pnext)
        {
        ...
        if (tx.AcceptToMemoryPool(txdb, &fMissingInputs))
        {
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                ...
                if (itByPrev == mapOrphanTransactionsByPrev.end())
                    continue;
                for (const uint256& orphanTxHash : itByPrev->second)
                {
                    ...
                    else if (!fMissingInputs2)
                    {
                        // invalid orphan
                        vEraseQueue.push_back(orphanTxHash);
                        LogPrint(BCLog::MEMPOOL, "   removed invalid orphan tx %s\n", orphanTxHash.ToString().substr(0,10).c_str());
                    }
                }
            }
    ...
    else if (strCommand == "getaddr")
    {
        ...
        if (!pfrom->fInbound)
        {
            LogPrint(BCLog::NET, "Ignoring \"getaddr\" from outbound connection. peer=%s\n", pfrom->addr.ToString().c_str());
            return true;
        }
    ...
    else if (strCommand == "pong")
    {
        ...
        if (!(sProblem.empty())) {
            LogPrint(BCLog::NET, "pong %s %s: %s, %" PRIx64" expected, %" PRIx64" received, %" PRIszu " bytes\n"
                , pfrom->addr.ToString().c_str()
...
{
    //if (fDebug)
    //    printf("ProcessMessages(%" PRIszu " messages)\n", pfrom->vRecvMsg.size());
    ...
    while (!pfrom->fDisconnect && it != pfrom->vRecvMsg.end()) {
        ...
        //if (fDebug)
        //    printf("ProcessMessages(message %" PRIszu " msgsz, %" PRIszu " bytes, complete:%s)\n",
