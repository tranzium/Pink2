...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("pinkcoin"))
        return false;
    ...
    for (auto& item : items)
    {
        ...
        if (item.first.startsWith("req-"))
        {
            item.first.remove(0, 4);
        ...
        else if (item.first == "amount")
        {
            if(!item.second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::BTC, item.second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert pinkcoin:// to pinkcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("pinkcoin://"))
    {
        uri.replace(0, 12, "pinkcoin:");
    }
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Pinkcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  pinkcoin-qt [" + tr("command-line options") + "]                     " + "\n";
