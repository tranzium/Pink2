...
WalletModel::WalletModel(CWallet *wallet, OptionsModel *optionsModel, QObject *parent) :
    ...
    cachedBalance(0), cachedTotalMinted(0), cachedStake(0), cachedUnconfirmedBalance(0), cachedImmatureBalance(0),
...
qint64 WalletModel::getConfirmingBalance() const
{
    return wallet->GetConfirmingBalance();
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    qint64 newTotalMinted = getTotalMinted();
    qint64 newStake = getStake();
    qint64 newUnconfirmedBalance = getUnconfirmedBalance();
    qint64 newConfirmingBalance = getConfirmingBalance();
    qint64 newImmatureBalance = getImmatureBalance();

    if(cachedBalance != newBalance || cachedTotalMinted != newTotalMinted || cachedStake != newStake || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedConfirmingBalance != newConfirmingBalance || cachedImmatureBalance != newImmatureBalance)
    {
        cachedBalance = newBalance;
        cachedStake = newStake;
        cachedUnconfirmedBalance = newUnconfirmedBalance;
        cachedConfirmingBalance = newConfirmingBalance;
        cachedImmatureBalance = newImmatureBalance;
        emit balanceChanged(newBalance, newTotalMinted, newStake, newUnconfirmedBalance, newConfirmingBalance, newImmatureBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(const QList<SendCoinsRecipient> &recipients, int nSplitBlock, const CCoinControl *coinControl)
{
...

    {
        ...
        int nChangePos = -1;
        bool fCreated = wallet->CreateTransaction(vecSend, wtx, keyChange, nFeeRequired, nChangePos, nSplitBlock, coinControl);
...
void WalletModel::subscribeToCoreSignals()
{
    ...
    wallet->NotifyAddressBookStakeChanged.connect(boost::bind(NotifyAddressBookStakeChanged, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3, boost::placeholders::_4, boost::placeholders::_5));
...
void WalletModel::unsubscribeFromCoreSignals()
{
    ...
    wallet->NotifyAddressBookStakeChanged.disconnect(boost::bind(NotifyAddressBookStakeChanged, this, boost::placeholders::_1, boost::placeholders::_2, boost::placeholders::_3, boost::placeholders::_4, boost::placeholders::_5));
...
void WalletModel::listCoins(std::map<QString, std::vector<COutput> >& mapCoins) const
{
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        while (wallet->IsChange(cout.tx->vout[cout.i]) && !cout.tx->vin.empty() && wallet->IsMine(cout.tx->vin[0]))
        {
